...
async function getContextInfo(input, config, transcriptPath) {
...
async function getSessionTokens(transcriptPath, totalInputTokens) {
    try {
...
async function main() {
    try {
        ...
        const contextData = await getContextFromCache(actualTranscriptPath, config.context.maxContextTokens);
